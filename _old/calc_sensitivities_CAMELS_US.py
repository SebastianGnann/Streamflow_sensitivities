        ...
        if '05_model_output' in name:
            ...
            try:
                sens_P_alt2, sens_PET_alt2, R2_alt2, _, _ , _  = sig_SensitivityLog(
                    df_tmp["OBS_RUN"].values, df_tmp["date"].values, df_tmp["PRCP"].values,
                    df_tmp["PET"].values, use_delta=False, fit_intercept=True, plot_results=False)
            except:
                sens_P_alt2, sens_PET_alt2, R2_alt2 = np.nan, np.nan, np.nan

            sens_P_alt3, sens_PET_alt3, R2_alt3, _, _ , _  = sig_Sensitivity(
                df_tmp["OBS_RUN"].values, df_tmp["date"].values, df_tmp["PRCP"].values,
                df_tmp["PET"].values, use_delta=True, fit_intercept=True, plot_results=False)

            """
            # Budyko / Turc as a comparison
            sens_P_Budyko, sens_PET_Budyko, R2_Budyko, _ = sig_SensitivityBudyko(
                df_tmp["OBS_RUN"].values, df_tmp["date"].values, df_tmp["PRCP"].values,
                df_tmp["PET"].values, plot_results=False, use_delta=False, fit_intercept=False)

            sens_P_Budyko, sens_PET_Budyko, R2_Budyko, _ = sig_SensitivityBudyko(
                df_tmp["OBS_RUN"].values, df_tmp["date"].values, df_tmp["PRCP"].values,
                df_tmp["PET"].values, plot_results=False, use_delta=True, fit_intercept=False)

            sens_P_Budyko, sens_PET_Budyko, R2_Budyko, _ = sig_SensitivityBudyko(
                df_tmp["OBS_RUN"].values, df_tmp["date"].values, df_tmp["PRCP"].values,
                df_tmp["PET"].values, plot_results=False, use_delta=False, fit_intercept=True)

            sens_P_Budyko, sens_PET_Budyko, R2_Budyko, _ = sig_SensitivityBudyko(
                df_tmp["OBS_RUN"].values, df_tmp["date"].values, df_tmp["PRCP"].values,
                df_tmp["PET"].values, plot_results=False, use_delta=True, fit_intercept=True)
            """

            # dQ_dP, dQ_dPET = util_TurcPike.calculate_sensitivities(np.nanmean(df_tmp["PRCP"]),
            #                                      np.nanmean(df_tmp["PET"]), 2)
            ...
            # BFI5_control = sig_BFI(df_tmp["OBS_RUN"].values, df_tmp["date"].values, method='UKIH', parameters=[5])
            ...
            corr_P_PET_list.append(df_tmp["PRCP"].corr(df_tmp["PET"]))
            '''
...
df = pd.merge(df, df_hydro, on='gauge_id', how='left')
...
plt.tight_layout()
fig.savefig(figures_path + "sens_PET_map_CAMELS_US" + ".png", dpi=600, bbox_inches='tight')
plt.close()
...
fig.savefig(figures_path + "Sensitivity_plot_CAMELS_US.png", dpi=600, bbox_inches='tight')
...
axes.legend()
plt.grid()
fig.savefig(figures_path + "Sensitivity_aridity_plot_CAMELS_US.png", dpi=600, bbox_inches='tight')
...
fig.savefig(figures_path + "Sensitivity_comparison_CAMELS_US.png", dpi=600, bbox_inches='tight')

# compare sens_P to sens_P_Budyko etc.
fig,(ax1,ax2)=plt.subplots(1,2,figsize=(6,3),constrained_layout=True)
im1=ax1.scatter(df["sens_P"],df["sens_P_Budyko"],s=10,c="tab:blue",alpha=0.8,lw=0,label="dQ/dP")
ax1.set_xlabel("dQ/dP [-]")
ax1.set_ylabel("dQ/dP Budyko [-]")
ax1.set_xlim([-0.5, 2])
ax1.set_ylim([-0.5, 2])
ax1.grid()
ax1.legend()
im2=ax2.scatter(df["sens_PET"],df["sens_PET_Budyko"],s=10,c="tab:orange",alpha=0.8,lw=0,label="dQ/dPET")
ax2.set_xlabel("dQ/dPET [-]")
ax2.set_ylabel("dQ/dPET Budyko [-]")
ax2.set_xlim([-1.5, 1])
ax2.set_ylim([-1.5, 1])
ax2.grid()
ax2.legend()
...
# Calculate empirical elasticities using mean values
df["elas_P"] = df["sens_P"] * (df["mean_P"] / df["mean_Q"])
df["elas_PET"] = df["sens_PET"] * (df["mean_PET"] / df["mean_Q"])
fig = plt.figure(figsize=(5, 3), constrained_layout=True)
axes = plt.axes()
axes.scatter(df["aridity_control"], df["elas_P"], s=10, c="tab:blue", alpha=0.8, lw=0, label="ε(P)")
axes.scatter(df["aridity_control"], df["elas_PET"], s=10, c="tab:orange", alpha=0.8, lw=0, label="ε(PET)")
sum_elas = df["elas_P"] + df["elas_PET"]
axes.scatter(df["aridity_control"], sum_elas, color='grey', s=10, label='ε(P) + ε(PET)')
P_vec = np.linspace(0.01, 10, 100)
E0_vec = np.linspace(10, 0.01, 100)
dQdP, dQdE0 = util_TurcPike.calculate_sensitivities(P_vec, E0_vec, 2)
Q_vec = util_TurcPike.calculate_streamflow(P_vec, E0_vec, 2)
elas_P_theory = dQdP * (P_vec / Q_vec)  # Budyko-scaled theoretical elasticity
elas_PET_theory = dQdE0 * (E0_vec / Q_vec)
axes.plot(E0_vec/P_vec, elas_P_theory, color='tab:blue', linestyle='--', linewidth=2)
axes.plot(E0_vec/P_vec, elas_PET_theory, color='tab:orange', linestyle='--', linewidth=2)
axes.set_xlabel("Aridity [-]")
...
axes.legend()
plt.grid()
fig.savefig(figures_path + "Elasticity_aridity_plot_CAMELS_US.png", dpi=600, bbox_inches='tight')
