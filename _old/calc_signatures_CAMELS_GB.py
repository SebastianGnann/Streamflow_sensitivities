...
for id in df_attr["gauge_id"]:
    ...
    perc_complete = np.sum(~np.isnan(df_tmp["discharge_spec"].values)) / len(df_tmp["discharge_spec"].values)

    # calculate SWE and snow fraction
    df_snow = util_SnowModel.calculate_swe(df_tmp["precipitation"].values,
                                           df_tmp["date"].values,
                                           df_tmp["temperature"].values,
                                           plot_results=False)
    df_tmp["melt"] = df_snow["melt"]
    df_tmp["rain"] = df_snow["rain"]
    df_tmp["swe"] = df_snow["swe"]

    frac_snow = np.nanmean(df_tmp["melt"]) / np.nanmean(df_tmp["precipitation"])

    seasonality_index = calculate_seasonality_index(
        df_tmp["precipitation"].values, df_tmp["pet"].values, df_tmp["date"].values)

    # Python signatures - perhaps code in TOSSH
    # sensitivities
    sens_P, sens_PET, R2, nr_years, VIF, cor_PET_P = sig_Sensitivity(
        df_tmp["discharge_spec"].values, df_tmp["date"].values, df_tmp["precipitation"].values, df_tmp["pet"].values, plot_results=True)
    sens_P_alt, sens_PET_alt, R2_alt, _, _, _ = sig_Sensitivity(
        df_tmp["discharge_spec"].values, df_tmp["date"].values, df_tmp["precipitation"].values, df_tmp["pet"].values, use_delta=True, fit_intercept=True)
    sens_P_alt2, sens_PET_alt2, R2_alt2, _, _, _ = sig_SensitivityLog(
        df_tmp["discharge_spec"].values, df_tmp["date"].values, df_tmp["precipitation"].values, df_tmp["pet"].values, use_delta=False, fit_intercept=True)
    sens_P_storage, sens_PET_storage, sens_Q_storage, R2_storage, _ = sig_SensitivityWithStorage(
        df_tmp["discharge_spec"].values, df_tmp["date"].values, df_tmp["precipitation"].values, df_tmp["pet"].values)

    from functions.util_StepwiseRegression import stepwise_regression_plot
    stepwise_regression_plot(Q=df_tmp["discharge_spec"].values, P=df_tmp["precipitation"].values,
    ...
    BFI5 = run_tossh_function(r'D:/Matlab/TOSSH/TOSSH_code', 'sig_BFI', eng,
                              df_tmp["discharge_spec"].values, df_tmp["date"].values,
    ...
    Storage_Baseflow = run_tossh_function(r'D:/Matlab/TOSSH/TOSSH_code', 'sig_StorageFromBaseflow', eng,
                                          df_tmp["discharge_spec"].values, df_tmp["date"].values,
                                          df_tmp["precipitation"].values, df_tmp["pet"].values,
    ...
    mean_P_list.append(np.nanmean(df_tmp["precipitation"]))
    mean_PET_list.append(np.nanmean(df_tmp["pet"]))
    mean_Q_list.append(np.nanmean(df_tmp["discharge_spec"]))
    mean_T_list.append(np.nanmean(df_tmp["temperature"]))
...
df_attributes_hydroatlas = pd.read_csv("D:/Data/Caravan-Jan25-csv/attributes/camelsgb/attributes_hydroatlas_camelsgb.csv",
                                       sep=',')
df_attributes_caravan = pd.read_csv("D:/Data/Caravan-Jan25-csv/attributes/camelsgb/attributes_caravan_camelsgb.csv",
                                    sep=',')
...
# save results
df.to_csv(results_path + 'camels_GB_signatures.csv', index=False)
print("Finished saving data.")

df = pd.read_csv(results_path + 'camels_GB_signatures.csv')

# load TOSSH results
df_tossh = pd.read_csv("D:/Python/Climate_sensitivity_mountains/results/CAMELS_GB_test.txt", sep=',', skiprows=0,
                       encoding='latin-1')

fig, ax = plt.subplots(figsize=(4, 3), tight_layout=True)
line_x = np.linspace(-10, 10, 100)
line_y = line_x
im = ax.scatter(df["BaseflowRecessionK"], df_tossh["BaseflowRecessionK"], s=20, alpha=0.9, c=df["perc_complete"])
ax.set_xlabel("")
ax.set_ylabel("")
# ax.set_xlim([0., 1.])
# ax.set_xlim([0., 1.])
r, p = stats.spearmanr(df["BaseflowRecessionK"], df_tossh["BaseflowRecessionK"], nan_policy='omit')
