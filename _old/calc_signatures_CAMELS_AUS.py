...
df_loc = pd.read_csv(
    "D:/Data/CAMELS_AUS/02_location_boundary_area/02_location_boundary_area/location_boundary_area.csv",
    sep=',', skiprows=0, encoding='latin-1')
df_climate = pd.read_csv("D:/Data/CAMELS_AUS/05_hydrometeorology/05_hydrometeorology/ClimaticIndices.csv",
                         sep=',', skiprows=0, encoding='latin-1')
df_geology = pd.read_csv("D:/Data/CAMELS_AUS/04_attributes/04_attributes/CatchmentAttributes_01_Geology&Soils.csv",
                         sep=',', skiprows=0, encoding='latin-1')
df_topo = pd.read_csv("D:/Data/CAMELS_AUS/04_attributes/04_attributes/CatchmentAttributes_02_Topography&Geometry.csv",
                      sep=',', skiprows=0, encoding='latin-1')
df_landcover = pd.read_csv(
    "D:/Data/CAMELS_AUS/04_attributes/04_attributes/CatchmentAttributes_03_LandCover&Vegetation.csv",
    sep=',', skiprows=0, encoding='latin-1')
df_humans = pd.read_csv(
    "D:/Data/CAMELS_AUS/04_attributes/04_attributes/CatchmentAttributes_04_AnthropogenicInfluences.csv",
    sep=',', skiprows=0, encoding='latin-1')
df_other = pd.read_csv("D:/Data/CAMELS_AUS/04_attributes/04_attributes/CatchmentAttributes_05_Other.csv",
                       sep=',', skiprows=0, encoding='latin-1')
df_hydro = pd.read_csv("D:/Data/CAMELS_AUS/03_streamflow/03_streamflow/streamflow_signatures.csv",
                       sep=',', skiprows=0, encoding='latin-1')
...
# read time series
df_P = pd.read_csv(
    "D:/Data/CAMELS_AUS/05_hydrometeorology/05_hydrometeorology/01_precipitation_timeseries/precipitation_AWAP.csv",
    sep=',')
df_PET = pd.read_csv(
    "D:/Data/CAMELS_AUS/05_hydrometeorology/05_hydrometeorology/02_EvaporativeDemand_timeseries/et_morton_wet_SILO.csv",
    sep=',')
df_Tmin = pd.read_csv("D:/Data/CAMELS_AUS/05_hydrometeorology/05_hydrometeorology/03_Other/AWAP/tmin_AWAP.csv", sep=',')
df_Tmax = pd.read_csv("D:/Data/CAMELS_AUS/05_hydrometeorology/05_hydrometeorology/03_Other/AWAP/tmax_AWAP.csv", sep=',')
df_Q = pd.read_csv("D:/Data/CAMELS_AUS/03_streamflow/03_streamflow/streamflow_mmd.csv", sep=',')
...
for id in df_attr["gauge_id"]:
    ...
    perc_complete = np.sum(~np.isnan(df_tmp["Q"].values)) / len(
        df_tmp["Q"].values)
    ...
    sens_P, sens_PET, R2, nr_years, VIF, cor_PET_P = sig_Sensitivity(
        ...
        df_tmp["PET"].values, plot_results=False, wateryear="A-MAR")
    sens_P_alt, sens_PET_alt, R2_alt, _, _, _ = sig_Sensitivity(
        df_tmp["Q"].values, df_tmp["date"].values, df_tmp["P"].values,
        df_tmp["PET"].values, use_delta=True, fit_intercept=True, wateryear="A-MAR")
    sens_P_alt2, sens_PET_alt2, R2_alt2, _, _, _ = sig_SensitivityLog(
        df_tmp["Q"].values, df_tmp["date"].values, df_tmp["P"].values,
        df_tmp["PET"].values, use_delta=False, fit_intercept=True, wateryear="A-MAR")
    sens_P_storage, sens_PET_storage, sens_Q_storage, R2_storage, _ = sig_SensitivityWithStorage(
        df_tmp["Q"].values, df_tmp["date"].values, df_tmp["P"].values,
        df_tmp["PET"].values, wateryear="A-MAR")
    # add averaging procedure...

    # from functions.util_StepwiseRegression import stepwise_regression_plot
    # stepwise_regression_plot(Q=df_tmp["Q"].values, P=df_tmp["P"].values,
    #                         PET=df_tmp["PET"].values, t=df_tmp["date"].values, plot_results=True, fit_intercept=False)
    ...
    # BFI5_control = sig_BFI(df_tmp["Q"].values, df_tmp["date"].values, method='UKIH', parameters=[5])
...
df_attributes_hydroatlas = pd.read_csv(
    "D:/Data/Caravan-Jan25-csv/attributes/camelsaus/attributes_hydroatlas_camelsaus.csv",
    sep=',')
df_attributes_caravan = pd.read_csv("D:/Data/Caravan-Jan25-csv/attributes/camelsaus/attributes_caravan_camelsaus.csv",
                                    sep=',')
...
df["gauge_lon"] = df_attr["long_outlet"]
df["gauge_lat"] = df_attr["lat_outlet"]

# todo: compare Caravan to native attributes

# save results
df.to_csv(results_path + 'camels_AUS_signatures.csv', index=False)
print("Finished saving data.")

df = pd.read_csv(results_path + 'camels_AUS_signatures.csv')
