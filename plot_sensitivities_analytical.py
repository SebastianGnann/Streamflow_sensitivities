...
def plot_sensitivities():
    ...
    fig_bar_P_pvalues, axs_bar_P_pvalues = plt.subplots(3, 2, figsize=(5, 8), sharex=True, sharey=True)
    fig_bar_PET_pvalues, axs_bar_PET_pvalues = plt.subplots(3, 2, figsize=(5, 8), sharex=True, sharey=True)
    ...
    for j, noise in enumerate(noise_levels):
        for i, corr in enumerate(correlations):
            ...
            pvalues_P = {m: [] for m in methods if "Mult" in m}
            pvalues_PET = {m: [] for m in methods if "Mult" in m}
            r2_values = {m: [] for m in methods if "Mult" in m}
            ...
            for p_mean in p_means:
                for pet_mean in pet_means:
                    ...
                    #r2_P_method3 = model_multi.rsquared
                    y_pred = model_multi.predict(X_noncentered)
                    ss_res = ((y_noncentered - y_pred) ** 2).sum()
                    ss_tot = ((y_noncentered - y_noncentered.mean()) ** 2).sum()
                    r2_P_method3 = 1 - ss_res / ss_tot
                    ...
                    # re-transform to non-log
                    y_pred = log_model.predict(log_X_with_const)
                    ...
                    results.extend([
                        ...
                        {"aridity_index": aridity_index,
                         "method": "Nonpara.", "sens_P": sens_P_method1,
                    ...
                    # Store p-values and R2 for multivariate methods only (no ODR)
                    for method, pP, pPET, r2 in zip(
                            ["Mult. Reg. #1", "Mult. Reg. #2", "Mult. Reg. Log"],
                            [p_value_P_method3, p_value_P_method4, p_value_P_method5],
                            [p_value_PET_method3, p_value_PET_method4, p_value_PET_method5],
                            [r2_P_method3, r2_P_method4, r2_P_method5]
                    ):
            ...
            # Store averages for p-values (only multivariate)
            ...
            # Store averages for R2 values (only multivariate)
            ...
            ax_PET.set_xlabel(r"$E_p$/$P$ [-]")
            ax_PET.set_ylabel(r"$s_{Ep}$ [-]")
            ...
            ax_bar_P.tick_params(axis='x', rotation=90)
            ...
            ax_bar_PET.tick_params(axis='x', rotation=90)
            ax_bar_PET.set_ylim(-25, 25)

            # Barplots for p-values P (multivariate only)
            ax_bar_pval_P = axs_bar_P_pvalues[i, j]
            sns.barplot(x=list(avg_pvalues_P.keys()), y=list(avg_pvalues_P.values()),
                        ax=ax_bar_pval_P, palette=custom_palette, hue=list(avg_pvalues_P.keys()))
            ax_bar_pval_P.set_title(f"Corr={corr}, Noise={noise}")
            ax_bar_pval_P.tick_params(axis='x', rotation=90)
            ax_bar_pval_P.set_ylim(0, 0.1)  # Changed as requested

            # Barplots for p-values PET (multivariate only)
            ax_bar_pval_PET = axs_bar_PET_pvalues[i, j]
            sns.barplot(x=list(avg_pvalues_PET.keys()), y=list(avg_pvalues_PET.values()),
                        ax=ax_bar_pval_PET, palette=custom_palette, hue=list(avg_pvalues_PET.keys()))
            ax_bar_pval_PET.set_title(f"Corr={corr}, Noise={noise}")
            ax_bar_pval_PET.tick_params(axis='x', rotation=90)
            ax_bar_pval_PET.set_ylim(0, 0.1)  # Changed as requested

            # Barplots for R2 (multivariate only)
            ax_bar_r2_ = axs_bar_r2[i, j]
            sns.barplot(x=list(avg_r2.keys()), y=list(avg_r2.values()),
                        ax=ax_bar_r2_, palette=custom_palette, hue=list(avg_r2.keys()))
            ax_bar_r2_.set_title(f"Corr={corr}, Noise={noise}")
            ax_bar_r2_.tick_params(axis='x', rotation=90)
            ax_bar_r2_.set_ylim(0, 1)

    # todo: count fraction of positive PET sensitivities for each method
    ...
    df_avg_pvalues_pivot = df_avg_pvalues.pivot_table(index=["Panel", "Sensitivity"], columns="Method", values="Avg p-value").round(3)
    df_avg_r2_pivot = df_avg_r2.pivot_table(index=["Panel", "Method"], values="Avg R2").round(2)
    ...
    fig_bar_P_pvalues.tight_layout()
    fig_bar_PET_pvalues.tight_layout()
